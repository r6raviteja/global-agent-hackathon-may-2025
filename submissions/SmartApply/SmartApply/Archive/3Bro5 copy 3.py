...
async def main():
    async with await browser.new_context() as context:
        ...
        tasks = [
            f"""  
            **Rules**:  
            1. **Sequential Fill**: Process fields in DOM order (top-to-bottom). Never backtrack.  
            2. **Error Handling**: Attempt each field **twice** → if failed, leave blank and move on.  
            3. **Scroll & Focus**: After each field:  
            - Scroll up after completing current field.  
            - Wait 3 seconds to stabilize DOM.  
            - Focus next field explicitly (`field.focus()`).  

            **Steps**:  
            1. Navigate to `{job_url}`. Wait 2s. Click "Apply" if present.  
            2. **Fill Fields**:  
            - Use `{personal_details}` for personal info.  
            - Use `{cv_content}` for skills (2-3 sentences max).  
            - **Dropdowns**: Select by value (not visible text).  
            - **Files**: Upload directly: Never click on file upload button. 
                - Resume: `{resume_path}`  
                - Cover Letter: `{coverLetter_path}` (if exists).  
            3. For radio buttons (like relocation willingness), Call "Select radio button safely"
            4. **Checkboxes**: Click first matching option (e.g., "Accept Terms").  
            5. **On Error**: Call `"Ask user for information"` for missing data.  

            **Final Action**: Stop after last field. **Do not submit** (hand off to next agent).  
            """  
