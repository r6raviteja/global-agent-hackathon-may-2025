...
# --- User Data Functions (from st2.py) ---
def load_user_data():
...
def save_user_data(updated_data):
    with open("userData.py", "w") as f:
        f.write(f"user_data = {updated_data}")
...
def show_user_preferences():
    with st.sidebar:
        ...
        with st.expander("🔍 Current Saved Data"):
...
# Replace the title and preferences button section with this:
...
# Show preferences if toggled - keep the stop() if you want it modal
if 'show_preferences' in st.session_state and st.session_state.show_preferences:
    show_user_preferences()
    st.stop()  # Keep this if you want the preferences to block other content

# Modify the show_user_preferences function to always show content (no expander):
def show_user_preferences():
    with st.sidebar:
        st.title("User Preferences")
        data = load_user_data()
        ...
        # Show current data without needing to expand
